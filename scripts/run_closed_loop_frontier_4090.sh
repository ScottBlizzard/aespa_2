...
RUN_ID="${RUN_ID:-frontier_200x200_20260630_4090}"
...
ENV_SHORTS="${ENV_SHORTS:-car ball}"
...
run_one() {
  ...
  stem="phase_dsrl_closed_loop_fixed_cap_${RUN_ID}_${algo}_${env_short}_seed${seed}_${AUDIT_EPISODES}x${TEST_EPISODES}_server"
  ...
  (
    ...
    CUDA_VISIBLE_DEVICES="$gpu" python src/run_dsrl_closed_loop_fixed_cap_audit.py \
      --algo "$algo" \
      --env-id "$env_id" \
...
echo "RUN_ID=$RUN_ID"
echo "AUDIT_EPISODES=$AUDIT_EPISODES TEST_EPISODES=$TEST_EPISODES CAPS=$CAPS"
...
if [ "$FAILED" -ne 0 ]; then
  echo "At least one frontier job failed. See $LOG_DIR." >&2
  exit 1
fi

python scripts/summarize_closed_loop_frontier_outputs.py \
  outputs/phase_dsrl_closed_loop_fixed_cap_${RUN_ID}_*_server.json \
  --out-csv "analysis/paper_assets/table_dsrl_closed_loop_fixed_cap_${RUN_ID}.csv" \
  --out-md "outputs/phase_dsrl_closed_loop_fixed_cap_${RUN_ID}_summary.md"

echo "SUMMARY outputs/phase_dsrl_closed_loop_fixed_cap_${RUN_ID}_summary.md"
echo "CSV analysis/paper_assets/table_dsrl_closed_loop_fixed_cap_${RUN_ID}.csv"
