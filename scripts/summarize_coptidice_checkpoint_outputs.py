"""Summarize COptiDICE checkpoint pilot JSON outputs into CSV and Markdown."""
...
def load_rows(patterns: Iterable[str]) -> List[Dict[str, object]]:
    ...
    for path in paths:
        ...
        if payload.get("proposer") != "official_dsrl_coptidice_checkpoint_proposer_v1":
            continue
        ...
        rows.append(
            {
                ...
                "policy_logp_mean": float(test_diag["checkpoint_policy_logp_mean"]),
            }
        )
    return sorted(
        rows,
...
def write_outputs(rows: List[Dict[str, object]], out_csv: Path, out_md: Path) -> None:
    ...
    out_md.write_text(
        "# COptiDICE Checkpoint Pilot Summary\n\n"
        + "\n".join(lines)
        + "\n\n## Aggregate\n\n"
...
def main() -> None:
    ...
    parser.add_argument(
        "--out-csv",
        type=Path,
        default=ROOT / "analysis" / "paper_assets" / "table_dsrl_coptidice_checkpoint_pilots.csv",
    )
    parser.add_argument(
        "--out-md",
        type=Path,
        default=ROOT / "outputs" / "phase_dsrl_coptidice_checkpoint_summary.md",
    )
    args = parser.parse_args()
    patterns = args.pattern or ["outputs/phase_dsrl_coptidice_checkpoint*_server.json"]
    rows = load_rows(patterns)
    if not rows:
        raise SystemExit("No COptiDICE checkpoint pilot JSON files found.")
